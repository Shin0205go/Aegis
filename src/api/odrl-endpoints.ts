...
export function createODRLEndpoints(hybridEngine: HybridPolicyEngine): Router {
  ...
  const aiEngine = hybridEngine.getAIEngine ? hybridEngine.getAIEngine() : undefined;
  const nlConverter = new NLToODRLConverter(aiEngine || undefined);
  ...
  router.post('/convert/patterns/import', async (req: Request, res: Response) => {
    try {
      ...
      // importLearnedPatterns expects a JSON string with specific format
