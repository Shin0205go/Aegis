...
describe('Configuration Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    // Reset singleton (Config is not a singleton, no action needed)
  ...
  describe('Security Integration Tests', () => {
    ...
    it('should validate encryption configuration for audit logs', () => {
      ...
      expect(config.getConfig().secretKey).toBe('encryption-key-for-audit-logs-32-chars');
    });
  });
  ...
  describe('Configuration Validation Completeness', () => {
    ...
    it('should validate nested required fields', () => {
      ...
      // Security config
      expect(fullConfig.secretKey).toBeDefined();
    });
  });
});